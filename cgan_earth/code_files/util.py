...
def batch(imgs, labels, batch_size, img_length, device):
    ...
    for i in range(n_img):
        for _ in range(n_crop):
            data = np.append(data, crop(imgs[i], img_length))
            batch_labels = np.append(batch_labels, labels[i])
    n_excess = batch_size - n_crop
    # randomly select an image and crop n_excess number of times
    if n_excess > 0:
        random_i = np.random.randint(0, n_img+1)
        random_img = imgs[random_i]
        for _ in range(n_excess):
            data = np.append(data, crop(random_img, img_length))
            batch_labels = np.append(batch_labels, random_i)
    # shuffle along dim 0
